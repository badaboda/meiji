...
class SequanceMatcherTest(unittest.TestCase):
    def test_x(self):
        s=SequenceMatcher(None, [(1, 1),(2, 2),(3, 3)], [(2, 2),(3, '3modified'),(4, 4)])
        for o in s.get_opcodes():
            print o
    def test_y(self):
        s=SequenceMatcher(None, [(1, 1)], [(1,1)])
        for o in s.get_opcodes():
            print o
...
class DeltaGeneratorWithKeyedDatumTest(DeltaGeneratorTest):
    KEYED_JSON_PATH="registry:players:**pcode:profile"
...
class DeltaGeneratorWithListDatum(DeltaGeneratorTest):
...
class DeltaGeneratorWithAtomDatum(DeltaGeneratorTest):
